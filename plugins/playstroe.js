...
var N_FOUND =''
   if (Config.LANG == 'EN') N_FOUND  = '*APK not found*'
   if (Config.LANG == 'SI') N_FOUND = '*ඔබ සෙවූ apk ගොනුව හමු නොවීය.*'
var NEED =''
   if (Config.LANG == 'EN') NEED  = '*Please enter a valid google play store apk link.\nyou can get it using .findapk command*'
   if (Config.LANG == 'SI') NEED = '*කරුණාකර වලංගු google play store app link එකක් ඇතුළත් කරන්න.\n.findapk විධානය මගින්ද එම ලින්ක් ලබා ගත හැකිය.*'
var DESC =''
...
Aqua.addrex({ pattern: 'findapk ?(.*)', fromMe: wk, desc:SDESC, deleteCommand: false }, async (message, match) => {
